...
import { extractUserData, isValidRBCAUserData, type RBCAUserData } from '@/types/rbca-user';
...
export const authOptions: AuthOptions = {
  ...
  providers: [
    GoogleProvider({
      ...
      clientSecret: secureEnv.get('GOOGLE_CLIENT_SECRET') || '',
    }),
    FacebookProvider({
      clientId: process.env.FACEBOOK_CLIENT_ID || '',
      clientSecret: secureEnv.get('FACEBOOK_CLIENT_SECRET') || '',
      authorization: {
        params: {
    ...
    LinkedInProvider({
      ...
      clientSecret: secureEnv.get('LINKEDIN_CLIENT_SECRET') || '',
      authorization: {
        params: {
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          const emailQuery = await database.ref('rbca_users')
          ...
          if (emailQuery.exists()) {
            ...
            const firebaseUserId = Object.keys(firebaseData)[SECURITY_CONFIG.FIRST_ELEMENT_INDEX];
            ...
            if (isValidRBCAUserData(userData)) {
              ...
              console.log(`🔍 [DEBUG] Found RBCA user in Firebase: ${firebaseUserId}`);
            } else {
              console.error('[SECURITY] Invalid user data structure in Firebase');
  ...
  session: {
    ...
    updateAge: SESSION_CONFIG.UPDATE_AGE_HOURS * SESSION_CONFIG.SECONDS_PER_HOUR, // 1 hour
  },
  jwt: {
    maxAge: SESSION_CONFIG.MAX_AGE_HOURS * SESSION_CONFIG.SECONDS_PER_HOUR, // 24 hours
  },
  callbacks: {
    async signIn({ user, account, profile, email, credentials }) {
      ...
      if (account && account.provider !== "credentials") {
        ...
        if (!allowedProviders.includes(account.provider)) {
          console.error("SECURITY: Invalid OAuth provider attempted:", account.provider);
          return false;
        }
        ...
        if (!rateLimitResult.allowed) {
          console.error("SECURITY: OAuth rate limit exceeded for provider:", account.provider);
          return false;
        }
        ...
        if (!nextAuthUserId) {
          console.error(`[SECURITY] ${account.provider} OAuth missing user.id and providerAccountId`);
          return false;
        }
        ...
        try {
          ...
          const existingUserSnapshot = await database.ref(`rbca_users/${firebaseUserId}`).once('value');
          ...
          const rbcaUserData: RBCAUserData = {
            ...
            pin: {
              ...
              fnm: (user.name && typeof user.name === 'string') ? user.name.split(' ')[NAME_PARSING.FIRST_NAME_INDEX]?.trim() || '' : '',
              lnm: (user.name && typeof user.name === 'string' && user.name.split(' ').length > NAME_PARSING.REMAINING_NAMES_START) ? user.name.split(' ').slice(NAME_PARSING.REMAINING_NAMES_START).join(' ').trim() : '',
              unm: '',
              str: '',
              cty: '',
              zip: '',
    ...
    async jwt({ token, user, account }) {
      ...
      if (user && user.id) {
        try {
          const database = getAdminDatabase();
          const userSnapshot = await database.ref(`rbca_users/${user.id}`).once('value');
          ...
          if (userSnapshot.exists()) {
            ...
            if (isValidRBCAUserData(userData)) {
              ...
              if (userData.pin?.eml && typeof userData.pin.eml === 'string' && userData.pin.eml.trim()) {
                token.email = userData.pin.eml.toLowerCase().trim();
              } else if (!token.email && user?.email && typeof user.email === 'string') {
              ...
              if (twoFactorRequired && userData.pin?.tfvA) {
                ...
                const TWO_FA_VERIFICATION_VALIDITY_MS = 5 * 60 * 1000; // 5 minutes
                const expiresAt = new Date(verifiedAt.getTime() + TWO_FA_VERIFICATION_VALIDITY_MS); // Calculate expiration
                const now = new Date();
        ...
        if (token.sub) {
          try {
            const database = getAdminDatabase();
            const userSnapshot = await database.ref(`rbca_users/${token.sub}`).once('value');
            ...
            console.error('[ERROR] Failed to check Firebase user on token refresh:', error);
          }
        }
      }
      return token;

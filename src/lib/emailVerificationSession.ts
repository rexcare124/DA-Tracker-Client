...
import { SESSION_CONFIG, VERIFICATION_TYPES } from "./emailVerificationConfig";
 ...
 */
export async function hasActiveVerificationSession(
  email: string,
): Promise<boolean> {
 ...
 */
export async function deactivateVerificationSession(
  sessionId: string,
): Promise<void> {
  // await prisma.emailVerification.updateMany({
  //   where: {
...
export async function hasExceededMaxAttempts(
  sessionId: string,
  maxAttempts: number = SESSION_CONFIG.MAX_ATTEMPTS_PER_SESSION,
): Promise<boolean> {

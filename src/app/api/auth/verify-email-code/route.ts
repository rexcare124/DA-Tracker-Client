...
const emailCodeVerificationSchema = z.object({
  ...
  code: z
    .string()
    .length(
      VERIFICATION_CODE_CONFIG.CODE_LENGTH,
      ERROR_MESSAGES.VALIDATION.INVALID_CODE_LENGTH,
    ),
});
...
export async function POST(request: Request) {
  try {
    ...
    const rateLimiter = createRateLimiter(
      RATE_LIMITS.EMAIL_VERIFICATION_ATTEMPTS,
    );
    ...
    if (!rateLimitResult.allowed) {
      return NextResponse.json(
        ...
        },
        {
          ...
          headers: {
            "X-RateLimit-Limit":
              RATE_LIMITS.EMAIL_VERIFICATION_ATTEMPTS.maxRequests.toString(),
    ...
    // const hasExceeded = await hasExceededMaxAttempts(
    //   emailVerification.sessionId!,
    // );
    // if (hasExceeded) {
    ...
    // console.log(
    //   `🔍 [PERF] Email verification update: ${queryTime}ms, fields: ${Object.keys(updateData).length}`,
    // );

  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        console.log("🔍 [DEBUG] Attempting RBCA login for email:", credentials.email.toLowerCase());
        ...
        try {
          ...
          console.log("🔍 [DEBUG] Firebase query result:", firebaseUsers ? Object.keys(firebaseUsers) : null);
          ...
          console.error("🔍 [ERROR] Firebase query failed during login:", firebaseError);
  ...
  callbacks: {
    ...
    async jwt({ token, user }) {
      ...
      if (user) {
        ...
        if (firebaseDatabase) {
          try {
            const firebaseUserSnapshot = await firebaseDatabase.ref(`rbca_users/${user.id}`).once('value');
            ...
            token.emailVerified = firebaseUser?.evf === true || firebaseUser?.emailVerified === true || false;
          } catch (error) {
            console.error("🔍 [ERROR] Failed to fetch Firebase user in JWT callback:", error);

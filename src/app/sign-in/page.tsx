...
const SignInPage: React.FC = () => {
  ...
  const [verificationExpiresAt, setVerificationExpiresAt] = useState<string | undefined>(undefined);
  const [verificationTimeRemaining, setVerificationTimeRemaining] = useState<number | undefined>(undefined);
  const [verificationAttempts, setVerificationAttempts] = useState<number | undefined>(undefined);
          ...
          if (errorMessage === "EMAIL_NOT_VERIFIED" || errorMessage.includes("EMAIL_NOT_VERIFIED")) {
            ...
            for (let attempt = 0; attempt < 2; attempt++) {
              try {
                ...
                console.error(`Session fetch attempt ${attempt + 1} failed:`, retryError);
                if (attempt === 1) {
            ...
            const sendVerificationCode = async () => {
              ...
              try {
                ...
                if (sendCodeResult.success) {
                  ...
                  if (sendCodeResult.status === 409 && sendCodeResult.isValidated) {
            ...
            if (sessionFetchSucceeded && sessionData) {
              if (sessionData.hasActiveSession && sessionData.timeRemaining && sessionData.timeRemaining > 0) {
  ...
  const handleEmailVerificationSuccess = useCallback(async () => {
    ...
    try {
      ...
      if (result?.ok) {
        ...
        const currentUser = session?.user as { twoFactorRequired?: boolean; twoFactorVerified?: boolean } | undefined;
        ...
        setError("Failed to sign in after email verification. Please try again.");
      }
    } catch {
      setError("An unexpected error occurred. Please try again.");
    } finally {
